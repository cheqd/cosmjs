...
FAUCET_API_KEY                Shared secret required for /credit and email OTP endpoints.
                              Requests must include the same value via the X-API-Key
                              header (or Authorization: Bearer <key>).
DB_URL                        Postgres DB URL to store request information.
MAILCHIMP_API_KEY             Mailchimp API key used to add users to contact lists.
MAILCHIMP_SERVER_PREFIX       Mailchimp server region prefix (e.g. 'us11')
MAILCHIMP_NEWSLETTER_LIST_ID  Mailchimp newsletter contacts list ID.
MAILCHIMP_PRODUCT_LIST_ID     Mailchimp product contacts list ID.
SENDGRID_FROM_EMAIL           Sendgrid from_email to send OTP codes from.
SENDGRID_OTP_TEMPLATE_ID      Sendgrid email template to use for OTP codes.
SENDGRID_API_KEY              Sendgrid API key.
OTP_SECRET                    Secret key to generate OTP hashes with.
```
